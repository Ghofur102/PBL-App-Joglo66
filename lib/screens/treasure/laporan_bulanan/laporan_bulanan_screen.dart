...
class _LaporanBulananScreenState extends State<LaporanBulananScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      setState(() => _laporanData = data);
    } catch (e) {
      setState(() => _errorMessage = 'Gagal memuat laporan: $e');
    } finally {
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              IconButton(
                icon:
                    const Icon(Icons.arrow_back, color: Color(0xFFB5D4F4)),
        ...
        child: Column(
          ...
          children: [
            ...
            Text(_errorMessage!,
                ...
                style:
                    const TextStyle(color: Color(0xFF444441), fontSize: 13)),
  ...
  Widget _buildRingkasanCards() {
    ...
    final isProfit =
        netProfit == null ? true : (netProfit as num) >= 0;
    ...
    return Column(
      children: [
        ...
        _summaryCard(
          ...
          iconColor:
              isProfit ? _primaryBlue : const Color(0xFF854F0B),
          valueColor:
              isProfit ? _primaryBlue : const Color(0xFF854F0B),
          fullWidth: true,
        ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _neracaRow(
              'Total Pemasukan', _laporanData?['total_income'],
              isPositive: true),
          _neracaRow(
              'Total Pengeluaran', _laporanData?['total_expense'],
              isPositive: false),
          const Divider(height: 20, color: Color(0xFFF1EFE8)),
          _neracaRow('Laba Bersih', _laporanData?['net_profit'],
              isBold: true),
          const SizedBox(height: 4),
          Text(
            'Dibuat: ${_laporanData?['generate_at'] ?? '-'}',
  ...
  Widget _neracaRow(String label, dynamic amount,
      {bool isPositive = true, bool isBold = false}) {
    final isNetProfit = isBold;
        ...
        children: [
          Text(label,
              style: TextStyle(
                  fontSize: 12,
                  color:
                      isBold ? const Color(0xFF2C2C2A) : _textSecondary,
                  fontWeight:
                      isBold ? FontWeight.w600 : FontWeight.normal)),
          Text(
            '${isPositive || isBold ? '' : '- '}${_formatRupiah(amount)}',
            style: TextStyle(
                fontSize: 13,
                fontWeight:
  ...
  Widget _buildExpenseBreakdown() {
    final expenses =
        _laporanData?['expenses'] as List<dynamic>? ?? [];
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    height: 8,
                    decoration: BoxDecoration(
                      color: const Color(0xFFA32D2D).withOpacity(0.6),
                  ...
                  Expanded(
                    child: Text(
                      ...
                      style: const TextStyle(
                          fontSize: 12, color: Color(0xFF2C2C2A)),
                    ),
                  ),
                  Text(
  ...
  Widget _buildUnduhButton() {
    return SizedBox(
      ...
      child: ElevatedButton.icon(
        onPressed: _laporanData == null
            ? null
            : () {/* TODO: implementasi unduh PDF/Excel */},
